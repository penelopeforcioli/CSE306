...
std::vector<Vector> Tutte(const TriangleMesh &mesh) {
    ...
    // Jasmine watissee and Victor Lasocki helped me a lot on the identification 
    // of the boundary, that is the part that is delimited here 
    ...
    std::map<int, int> map;
    for (std::map<std::pair<int, int>, int>::iterator i = count.begin(); i != count.end(); ++i) { 

    if (i->second == 1) { 
        adjacent_vertices[i->first.second].push_back(i->first.first);
        map[i->first.first] = i->first.second;
    }
}


    // ORDER BOUNDARY VERTICES
    for (int p = map.begin()->first, i = 0; i < map.size(); p = map[p], i++) {
        boundary_flag[p] = true;
        b.push_back(p);
    }
